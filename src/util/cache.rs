	...
	path.join("._cache")
		...
		.then(|| {
			let cache = ResourceCache::load(
				fs::read_to_string(path.join("._cache")).nice_unwrap("Unable to read cache"),
			);
			Some(CacheBundle {
				cache,
	...
	path.exists()
		.then(|| {
			match zip::ZipArchive::new(File::open(path).nice_unwrap("Unable to open cached package")) {
				Ok(mut archive) => {
					let cache: ResourceCache = if archive.by_name("._cache").is_ok() {
...
impl ResourceCache {
	...
	pub fn add_sheet(&mut self, sheet: &SpriteSheet, path: PathBuf) {
		if !path.is_relative() {
			unreachable!("Contact  developers: {}", path.display());
		}
